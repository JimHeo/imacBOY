...
def split_validation(sample_list, model, percentage=0.2, epochs=20, evaluation_path="evaluations",
    ...
    cb_model = ModelCheckpoint(os.path.join(evaluation_path, "model.hdf5"), monitor="val_f1_score", verbose=1, save_best_only=True, mode="max")
    if save_models == True : cb_list = callbacks + [cb_model]
    else : cb_list = callbacks
    history = model.evaluate(training, validation, epochs=epochs, callbacks=cb_list)
